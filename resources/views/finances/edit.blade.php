                            ...
                            <div>
                                ...
                                <div class="mb-3 p-3 bg-gray-50 rounded-lg">
                                    ...
                                    @if(Str::endsWith($finance->receipt_file, '.pdf'))
                                        <a href="{{ asset('storage/' . $finance->receipt_file) }}" target="_blank" class="text-blue-600 hover:text-blue-800">
                                            📄 Lihat PDF
                                        </a>
                                    @else
                                        <img src="{{ asset('storage/' . $finance->receipt_file) }}" alt="Bukti" class="max-w-xs rounded border">
                                    @endif
                                </div>
                                @endif
                                <input type="file" name="receipt_file" accept="image/*,.pdf"
                                    class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                                <p class="mt-1 text-sm text-gray-500">Upload file baru jika ingin mengganti</p>
                                @error('receipt_file')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
