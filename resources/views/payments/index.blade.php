    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('payments.index') }}" class="btn-success btn-sm flex-1 sm:flex-none">
                                ...
                                <td>
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        @if($payment->tenant->phone)
                                            <button type="button"
                                                    onclick="sendWhatsApp('{{ $payment->uuid }}', '{{ $payment->tenant->name }}')"
                                        ...
                                        <form id="delete-payment-{{ $payment->id }}" action="{{ route('payments.destroy', $payment) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="button" onclick="confirmDelete(event, 'delete-payment-{{ $payment->id }}', '{{ $payment->invoice_number }}')" class="p-1.5 text-stone-400 transition hover:text-red-600" title="Hapus">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            @if(session('new_payment_id'))
                ...
                @if($newP)
                    ...
                    sendWhatsApp('{{ $newP->uuid }}', '{{ $newP->tenant->name }}');
                @endif
            @endif
        });
    </script>
